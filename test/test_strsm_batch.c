 ...
 *
 * @generated from test/test_ztrsm_batch.c, normal z -> s, Sat Oct 27 15:01:40 2018
 *
 **/
#include "test.h"
#include "flops.h"
...
void test_strsm_batch(param_value_t param[], bool run)
{
	...
	for (int group_iter= 0; group_iter < group_count; group_iter++) {
		...
		for (int matrix_iter= group_start; matrix_iter < group_end; matrix_iter++) {
			...
			retval = LAPACKE_slarnv(1, seed, (size_t)lda[group_iter]*n[group_iter], 
					A[matrix_iter]);
			assert(retval == 0);
	...
	blas_strsm_batch(group_count, (const int *)group_sizes,
	...
	if (test) {
		...
		for (int group_iter= 0; group_iter < group_count; group_iter++) {
			...
			for (int matrix_iter= group_start; matrix_iter < group_end; matrix_iter++) {
				...
				cblas_strsm(CblasColMajor, (CBLAS_SIDE)side[group_iter], (CBLAS_UPLO)uplo[group_iter],
