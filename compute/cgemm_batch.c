 ...
 *
 * @generated from compute/zgemm_batch.c, normal z -> c, Sat Oct 27 15:01:40 2018
 *
 **/
 ...
 * @ingroup gemm_batch
 *
 * blas_cgemm_batch is a batch version of cgemm. It performs 
		      ...
		      int *info)
{
	...
	for (int group_iter = 0; group_iter < group_count; group_iter++) {
		...
		if (m[group_iter] == 0 || n[group_iter] == 0 ||
				((alpha[group_iter] == (bblas_complex32_t)0.0 || 
				 k[group_iter] == 0) && 
				 beta[group_iter] == (bblas_complex32_t)1.0 ) ||
				 group_sizes[group_iter] == 0) {
			bblas_success(info_option, &info[info_offset], group_sizes[group_iter]);
			continue;
		}

		// Call to blas_cgemm_batchf 
		blas_cgemm_batchf(group_sizes[group_iter], 
