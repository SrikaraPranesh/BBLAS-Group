 ...
 *
 * @generated from compute/zsyr2k_batch.c, normal z -> s, Sat Oct 27 15:01:39 2018
 *
 **/
 ...
 *          the symmetric matrices C[j]-s of i-th group are to be stored
 *  
...

{
	...
	for (int group_iter = 0; group_iter < group_count; group_iter++) {
		...
		if (n[group_iter] == 0 || k[group_iter] == 0 ||
				(alpha[group_iter] == (float)0.0 || 
				 beta[group_iter] == (float)1.0) || 
				group_sizes[group_iter] == 0) {
			bblas_success(info_option, &info[info_offset], group_sizes[group_iter]);
			continue;
		}

		// Call to blas_ssyr2k_batchf 
		blas_ssyr2k_batchf(group_sizes[group_iter], 
